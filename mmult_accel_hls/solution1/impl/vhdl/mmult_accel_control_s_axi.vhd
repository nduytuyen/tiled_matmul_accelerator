...
port (
    ...
    update_A              :out  STD_LOGIC_VECTOR(31 downto 0);
...
architecture behave of mmult_accel_control_s_axi is
    ...
    constant ADDR_K_DATA_0        : INTEGER := 16#3c#;
    constant ADDR_K_CTRL          : INTEGER := 16#40#;
    constant ADDR_M_DATA_0        : INTEGER := 16#44#;
    constant ADDR_M_CTRL          : INTEGER := 16#48#;
    constant ADDR_UPDATE_A_DATA_0 : INTEGER := 16#4c#;
    constant ADDR_UPDATE_A_CTRL   : INTEGER := 16#50#;
    ...
    signal int_update_A        : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_UPDATE_A_DATA_0 =>
                        rdata_data <= RESIZE(int_update_A(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    update_A             <= STD_LOGIC_VECTOR(int_update_A);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_UPDATE_A_DATA_0) then
                    int_update_A(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_update_A(31 downto 0));
                end if;
            end if;
        end if;
    end process;
